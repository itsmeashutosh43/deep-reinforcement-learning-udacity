...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        #double q learning
        # selection of action from online weights : qnetwork_local
        # getting q vlues from : qnetwork_target
       
        states, actions, rewards, next_states, dones = experiences
        
        best_action_arg = self.qnetwork_local(next_states).detach()
        a_best = best_action_arg.max(1)[1]
        Q_targets_all = self.qnetwork_target(next_states).detach()
        ...
        _, a_prime = Q_argmax.max(1) # no backpropagation up from here, so used over detachhh
